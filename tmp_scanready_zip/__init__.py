...
SCANREADY_BLENDER_VERSION = (4, 0, 0)
...
def ensure_cycles_render_engine(context, operator=None, props=None):
    ...
    if not cycles_is_available():
        message = "Cycles render engine is not available. Enable Blender's Cycles add-on in Preferences > Add-ons, then restart Blender and try again."
    ...
    try:
        ...
        message = f"Could not switch to Cycles render engine: {exc}"
        if props is not None:
            log_progress(props, 1, message)
        if operator is not None:
            operator.report({'ERROR'}, message)
...
def clean_high_scan_mesh(obj, weld_distance_cm=0.0):
    if obj is None or obj.type != 'MESH':
        return
    ...
    # Pre-weld the source scan before loose/island cleanup. Photogrammetry
    # assets can contain valid shell pieces separated by tiny gaps; welding first
    # prevents them from being treated as floating debris.
    try:
        weld_distance = max(0.0, float(weld_distance_cm)) / 100.0
    except Exception:
        weld_distance = 0.0
    ...
    if weld_distance > 0.0:
        try:
            ...
            valid_verts = [v for v in bm.verts if v.is_valid]
            if valid_verts:
                bmesh.ops.remove_doubles(bm, verts=valid_verts, dist=weld_distance)
                bmesh.update_edit_mesh(obj.data)
        except Exception:
            pass
...
class ALB_OT_create_preview(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            source_is_selected = False
        if (source is None or source.type != 'MESH' or not source_is_selected):
            source = find_existing_high_mesh_in_scene(context)
            source_is_selected = source is not None

        if source is None or source.type != 'MESH' or not source_is_selected:
            log_progress(props, 0, "Select the high mesh.")
            try:
                props.progress_detail = ""
            except Exception:
                pass
        ...
        clean_high_scan_mesh(source, props.weld_distance_cm)
...
def reveal_source_mesh_for_bake(obj):
    """Reveal hidden source geometry without changing UV material isolation."""
    if obj is None or getattr(obj, "type", None) != 'MESH':
        return
    try:
        ensure_object_mode()
...
def force_bake_source_target_state(context, high_obj, uv_obj):
    """Make selected-to-active bake state explicit immediately before baking."""
    scene = context.scene
    ensure_object_mode()
    exit_local_view_if_needed(context)
    ensure_object_visible_for_bake(high_obj)
...
class ALB_OT_bake_textures_modal(Operator):
    ...
    def _run_single_bake(self, context, bake_type, mat_idx):
        ...
        if bake_type == 'AO':
            ...
            force_bake_source_target_state(context, high_obj, uv_obj)
