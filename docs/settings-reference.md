...
**Use Texture View** is available in **Advanced > Mesh Settings** because it is not usually adjusted during the main workflow.
...
| **Force CPU Baking** | Forces baking on CPU instead of GPU. | Off by default for one material. Automatically enabled when Bake Materials is `2` or higher, and can be disabled manually. |
